...
if ((strpos($output, "niteflirt.com/login") !== false) && (strpos($output, "redirected") !== false)) {
	...
	if ($config["debug"]) { echo date(DATE_RFC2822) . ": logging in\n"; }
	...
	if (preg_match('/<input\stype="hidden"\sname="authenticity_token"\svalue="(.*?)"\s\/>/', $output, $matches) === 1) { $form_post["authenticity_token"] = $matches[1]; }
	...
	if ($config["debug"]) { echo date(DATE_RFC2822) . ": login successful\n"; }
}
...
if (preg_match('/Your\scurrent\sbalance\sis\s\$([\d\.]+)</', $output, $matches) === 1) { $balance = $matches[1]; } else { die("Balance is not detected on current page"); }
...
if (preg_match_all('/<td\sclass="bid">(?:[\w\s()]+?)?\$([\d\.]+)<\/td>/', $output, $matches) > 0) {
	...
	foreach ($matches[0] as $idx => $is_current_bid)
	{
		...
		{
			if ($config["debug"]) { echo date(DATE_RFC2822) . ": I see our current bid and position\n"; }
...
/* If we are currently in our position, we're good. */
if ($config["position"] == $my_position)
{
	if ($config["debug"]) { echo date(DATE_RFC2822) . ": We are where we want to be\n"; }
	die();
}

/* To "be" in that position we have to bid a penny more. Might want to make this random in the future */
$bid_amt = $bids[$config["position"] - 1] + 0.01;
if ($config["debug"]) { echo date(DATE_RFC2822) . ": For position " . $config["position"] . " we have to bid $bid_amt\n"; }
...
if (preg_match('/<input\stype="hidden"\sname="authenticity_token"\svalue="(.*?)"\s\/>/', $output, $matches) === 1) { $form_post["authenticity_token"] = $matches[1]; }
if (preg_match('/<input\stype="hidden"\sname="category_id"\sid="category_id"\svalue="(.*?)"\s\/>/', $output, $matches) === 1) { $form_post["category_id"] = $matches[1]; }
...
if ($config["debug"]) { echo date(DATE_RFC2822) . ": Bidding / ready to POST to $post_action\n"; }
curl_setopt($ch, CURLOPT_POST, true);
...
if (preg_match('/<input\stype="hidden"\sname="authenticity_token"\svalue="(.*?)"\s\/>/', $output, $matches) === 1) { $form_post["authenticity_token"] = $matches[1]; }
if ($config["debug"]) { echo date(DATE_RFC2822) . ": Confirming / ready to POST to $post_action\n"; }
curl_setopt($ch, CURLOPT_POST, true);
curl_setopt($ch, CURLOPT_URL, $post_action);
curl_setopt($ch, CURLOPT_POSTFIELDS, http_build_query($form_post));
curl_setopt($ch, CURLOPT_FOLLOWLOCATION, true);
...
{
	if ($config["debug"]) { echo date(DATE_RFC2822) . ": We are Congratulated\n"; }
} else {
	if ($config["debug"]) { echo date(DATE_RFC2822) . ": We are confused\n"; }
}

/* All done */
curl_close($ch);
if ($config["debug"]) { echo date(DATE_RFC2822) . ": All Done!\n"; }
