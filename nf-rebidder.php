...
/**
 * nf-rebidder.php
 * Automatically align all your listings to a specific position by optimally bidding on them
 * 
 * Configuration
 * "listings": array of arrays
 *   "url": The niteflirt URL that allows you to bid on your listing, typically ends with ?category_id=
 *   "position": The position you'd like this listing to bid to. 1-12 gets you on the first page.
 * "login_name": Your email address/login name to log into niteflirt
 * "login_password": Your password to log into niteflirt
 * "highbid": (true|false) When bidding, squeeze closer to POSITION-1 instead of overbid POSITION by 0.01
 * "biddown": (true|false) If we are positionally in a higher place and we want to be lower, make the lower bid
 * "do_bid": (true|false) If you want this to actually do the bidding, set to true.
 * "debug": (true|false) Debug output to stdout
 */
$config = [
	"listings" => [
		[
			"url" => "https://www.niteflirt.com/account/featured_listings/123456-some-listing-name-is-here?category_id=789",
			"position" => 5,
		],
		[
			"url" => "https://www.niteflirt.com/account/featured_listings/987654-another-listing-name-is-here?category_id=123",
			"position" => 10,
		],
	],
	"login_name" => "",
...
foreach ($config["listings"] as $listing)
{
	/* simplify what we have in the loop */
	$cfg_listing = $listing["url"];
	$cfg_position = $listing["position"];

	echo date(DATE_RFC2822) . ": Getting $cfg_listing\n";
	/* Get the configured page */
	$ch = curl_init();
	curl_setopt($ch, CURLOPT_URL, $cfg_listing);
	...
	if (!$config["biddown"])
	{
		if (($my_position > 0) && ($my_position <= $cfg_position))
		{
			if ($config["debug"])
			{
	...
	else
	{
		if ($config["highbid"])
		{
			 ...
			 */
			$bid_amt = $bids[$cfg_position - 2] - 0.01;
		}
		else
		{
			/* At this point we butt up again next place. This approach is more volatile (like 1st place) and would require more shifts */
			$bid_amt = $bids[$cfg_position - 1] + 0.01;
		}
	}
	if ($config["debug"])
	{
		echo date(DATE_RFC2822) . ": For position " . $cfg_position . " we have to bid $bid_amt\n";
	}
	...
	else
	{
		if ($config["debug"])
		{
			echo date(DATE_RFC2822) . ": We have do_bid=false, not bidding\n";
		}
	}
	curl_close($ch);
}
